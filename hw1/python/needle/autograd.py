...
class Op:
    ...
    # 求y对于所有node.inputs即输入Tensor的梯度（partial adjoint），并以tuple类型返回
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      ...
      # 计算node.inputs -> node的partial adjoint，即根据chain rule
      # 由dy/d(node.inputs[i]) = dy/d(node) * d(node)/d(node.inputs[i])
      # 其中dy/d(node) == node.grad
